...
<body>
	...
	<br>
	<div class="container">
		<h3>Parcours liés à votre recherche</h3>
		<br>
		...
		<c:forEach items="${listParcours}" var="parcours">
        	...
        	Métiers: ${parcoursJobs.get(parcours.key)}
					...
					<c:if test="${!empty parcoursNotes}">
						<c:forEach items="${parcoursNotes.get(parcours.key)}"
							var="prerequis">
							<font color="${prerequis.value}">${prerequis.key.name}
								(${prerequis.key.requis})</font>
							<br>
						</c:forEach>
					</c:if>
				</div>
